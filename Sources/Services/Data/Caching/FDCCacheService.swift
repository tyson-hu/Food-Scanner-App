...
struct CacheConfiguration {
    ...
    nonisolated static let `default` = CacheConfiguration(
        ...
        maxSize: 1_000 // Maximum number of cached items
    )
}
...
final class FDCCacheService: ObservableObject {
    ...
    private var paginatedSearchCache: [String: CacheEntry<FoodDataSearchResult>] = [:]
    ...
    func cachedPaginatedSearchResults(for query: String, page: Int, pageSize: Int) -> FoodDataSearchResult? {
    ...
    func cachePaginatedSearchResults(_ results: FoodDataSearchResult, for query: String, page: Int, pageSize: Int) {
        let key = normalizedPaginatedQuery(query, page: page, pageSize: pageSize)
