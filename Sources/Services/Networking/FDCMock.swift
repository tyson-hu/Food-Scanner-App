...
struct FDCMock: FoodDataClient {
    ...
    func searchFoodsWithPagination(
        ...
        let trimmed = query.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else {
            return FoodDataSearchResult(foods: [], totalHits: 0, currentPage: page, totalPages: 0, pageSize: pageSize)
        }
        ...
        let foods = slice.map { createFDCFoodSummary(from: $0) }
    ...
    private func createFDCFoodSummary(from food: FDCFoodDetails) -> FDCFoodSummary {
        FDCFoodSummary(
            id: food.id,
            name: food.name,
            brand: food.brand,
            ...
            cholesterol: nil,
            sodium: nil,
            carbohydrates: Double(food.carbs),
            fiber: nil,
            sugars: nil,
            calcium: nil,
            iron: nil,
    ...
    func fetchFoodDetailResponse(fdcId: Int) async throws -> ProxyFoodDetailResponse {
        ...
        let mockNutrients = [
            ProxyFoodNutrient(
                nutrient: ProxyNutrientInfo(id: 1_008, number: "208", name: "Energy", rank: 300, unitName: "kcal"),
                amount: 100.0,
                dataPoints: 1,
                derivation: ProxyDerivation(
                    id: 1,
                    code: "LCCD",
                    description: "Calculated from a daily value percentage per serving size measure",
                    sourceDescription: "Calculated from a daily value percentage per serving size measure"
                ),
                type: "FoodNutrient"
            ),
            ProxyFoodNutrient(
                nutrient: ProxyNutrientInfo(id: 1_003, number: "203", name: "Protein", rank: 600, unitName: "g"),
                amount: 7.0,
                dataPoints: 1,
                derivation: nil,
                type: "FoodNutrient"
        ...
        if let hit = Self.catalog.first(where: { $0.id == fdcId }) {
            ...
            let labelNutrients = ProxyLabelNutrients(
                calories: ProxyNutrientValue(value: Double(hit.calories), unit: "kcal"),
                fat: ProxyNutrientValue(value: Double(hit.fat), unit: "g"),
                saturatedFat: nil,
                transFat: nil,
                cholesterol: nil,
                sodium: nil,
                carbohydrates: ProxyNutrientValue(value: Double(hit.carbs), unit: "g"),
                fiber: nil,
                sugars: nil,
                protein: ProxyNutrientValue(value: Double(hit.protein), unit: "g"),
                calcium: nil,
                iron: nil,
                potassium: nil
            )
